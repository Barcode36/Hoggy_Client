 ...
 * @author Fallou
 */
public class AddMedecin implements Initializable {
    ...
    private final FilteredList<Medecin> filteredData = new FilteredList<>(data, p -> true);
    private final SortedList<Medecin> sortedData = new SortedList<>(filteredData);
    @FXML
    private Label today;
   @FXML
    private Label photo;
  ...
  FileChooser f = new FileChooser();
    ...
    @FXML
    private JFXButton modifiertbx;String mois;
File files =new File("src/images/patientimage.jpg");
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
          mois=mois();
            ...
            imv2.setFitHeight(92);
            imv2.setFitWidth(92);
            cercle.setFill(new ImagePattern(image2));
...
public String mois() {
    ...
    public void activer() {
        try {
            ...
            Medecin et = (Medecin) table.getSelectionModel().getSelectedItem();
    ...
    public void LoadEtudiantFromBase() {
        try {
            ...
            List<Medecin> l = stub2.listeMedecin();
            List<Sexe>ls=stub.listeSexe();  sexecbx.getItems().addAll(ls);
            l.stream().filter((a) -> (a.getIdprofil().getId()==2)).forEachOrdered((a) -> {
                data.add(a);
            });
            table.setItems(data);
        } catch (Exception ex) {
     ...
     public void enregistrer() {
        ...
        try {
            ...
            if (matriculetbx.getText().trim().isEmpty() || nomtbx.getText().trim().isEmpty() || prenomtbx.getText().trim().isEmpty() || adressetbx.getText().trim().isEmpty() || naissancetbx.getValue() == null) {
              ...
              Specialite s=new Specialite(3,"General");
                m.setIdprofil(p);
          m.setIdspecialite(s);
       ...
       try {
            ...
            if (matriculetbx.getText().trim().isEmpty() || nomtbx.getText().trim().isEmpty() || prenomtbx.getText().trim().isEmpty() || adressetbx.getText().trim().isEmpty() ) {
               ...
               alert.showAndDismiss(Duration.seconds(2));
                                ...
                                Medecin m   = (Medecin) table.getSelectionModel().getSelectedItem();
