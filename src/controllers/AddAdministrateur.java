...
public class AddAdministrateur implements Initializable {
   @FXML
    ...
    private final FilteredList<Administrateur> filteredData = new FilteredList<>(data, p -> true);
    private final SortedList<Administrateur> sortedData = new SortedList<>(filteredData);
    @FXML
    private Label today;
...
File files =new File("src/images/login_user.png");
    @FXML
   ...
   public void deconnecter(){
    ...
    public void activer() {
        try {
            ...
            Administrateur et = (Administrateur) table.getSelectionModel().getSelectedItem();
    ...
    public void LoadEtudiantFromBase() {
        try {
            ...
            IUser stub = (IUser) Naming.lookup("rmi://localhost:1099/admin");
            List<Administrateur> l = stub.listeAdministrateur();
            List<Sexe>ls=stub.listeSexe();  sexecbx.getItems().addAll(ls);
            l.stream().filter((ad) -> (ad.getIdprofil().getId()==1)).forEachOrdered((ad) -> {
                data.add(ad);
            });
            ...
            if (matriculetbx.getText().trim().isEmpty() || nomtbx.getText().trim().isEmpty() || prenomtbx.getText().trim().isEmpty() || adressetbx.getText().trim().isEmpty() || naissancetbx.getValue() == null) {
              ...
              alert.setMessage("LE NUMERO TELEPHONE N EST PAS BON.IL DOIT COMMENCE PAR 77 78 76 70 OU 33");
               ...
               enregistrertbx.setVisible(true);
                ...
                Profil p = new Profil(1, "ADMINISTRATEUR");
                admin.setIdprofil(p);
                admin.setIdstatus(2);
               ...
               alert.showAndDismiss(Duration.seconds(2));
                ...
                if (ok == 0) {
                    ...
                    succes.setMessage("PERSONNE ENREGISTREE AVEC SUCCE");
               succes.showAndDismiss(Duration.seconds(2));
