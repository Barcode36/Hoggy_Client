...
public class AddSecretaire implements Initializable {
    ...
    private final ObservableList<Secretaire> data = FXCollections.observableArrayList();

    private final FilteredList<Secretaire> filteredData = new FilteredList<>(data, p -> true);
    private final SortedList<Secretaire> sortedData = new SortedList<>(filteredData);
    @FXML
    private Label today;
    ...
    @FXML
    private TableView<Secretaire> table;
 ...
 public void deconnecter(){
    ...
    public void activer() {
        try {
            ...
            Secretaire et = (Secretaire) table.getSelectionModel().getSelectedItem();
    ...
    public void LoadEtudiantFromBase() {
        try {
            ...
            IUser stub = (IUser) Naming.lookup("rmi://localhost:1099/admin");
              ISecretaire stub2 = (ISecretaire) Naming.lookup("rmi://localhost:1099/secretaire");
            List<Secretaire> l = stub2.listeSecretaire();
            List<Sexe>ls=stub.listeSexe();  sexecbx.getItems().addAll(ls);
            l.stream().filter((a) -> (a.getIdprofil().getId()==5)).forEachOrdered((a) -> {
                data.add(a);
            });
            table.setItems(data);
        } catch (Exception ex) {
            ...
            if (matriculetbx.getText().trim().isEmpty() || nomtbx.getText().trim().isEmpty() || prenomtbx.getText().trim().isEmpty() || adressetbx.getText().trim().isEmpty() || naissancetbx.getValue() == null) {
              ...
              alert.setMessage("LE NUMERO TELEPHONE N EST PAS BON.IL DOIT COMMENCE PAR 77 78 76 70 OU 33");
               ...
               enregistrertbx.setVisible(true);
                ...
                Profil p = new Profil(5, "SECRETAIRE");
                admin.setIdprofil(p);
                admin.setIdstatus(2);
       ...
       try {
            ...
            if (matriculetbx.getText().trim().isEmpty() || nomtbx.getText().trim().isEmpty() || prenomtbx.getText().trim().isEmpty() || adressetbx.getText().trim().isEmpty() ) {
               ...
               alert.showAndDismiss(Duration.seconds(2));
                        ...
                        Secretaire m   = (Secretaire) table.getSelectionModel().getSelectedItem();
